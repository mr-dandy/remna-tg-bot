...
class PanelApiService:
    ...
    def _unwrap_response(self, payload: Optional[Dict[str, Any]]) -> Optional[Any]:
        """Return inner response payload for both v1 (response) and v2 (data/plain) formats."""
    ...
    async def _request(self,
        ...
        async def _do_request(url: str) -> Dict[str, Any]:
            async with aiohttp_session.request(method.upper(), url, headers=headers, **kwargs) as response:
    ...
    async def get_user_by_uuid(
        ...
        unwrapped = self._unwrap_response(full_response)
    ...
    async def create_panel_user(
        ...
        if isinstance(unwrapped, dict):
            logging.info(
                f"Panel user '{username_on_panel}' created successfully (UUID: {unwrapped.get('uuid')})."
            )
            return {"response": unwrapped} if "response" not in (response or {}) else response
    ...
    async def update_user_details_on_panel(
        ...
        full_response = await self._request("PATCH", "/users", json=update_payload, log_full_response=log_response)
        unwrapped = self._unwrap_response(full_response)
        if unwrapped is not None:
    ...
    async def update_user_status_on_panel(self,
        ...
        unwrapped = self._unwrap_response(response_data)
        if isinstance(unwrapped, dict):
            actual_status = unwrapped.get("status")

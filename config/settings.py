...
class Settings(BaseSettings):
    ...
    # Support aliases LANG and LANGUAGE for DEFAULT_LANGUAGE and normalize value
    @field_validator('DEFAULT_LANGUAGE', mode='before')
    @classmethod
    def resolve_default_language(cls, v):
        # Prefer explicit value if provided; otherwise check common env aliases
        ...
        if isinstance(v, str):
            ...
            # keep only first part like 'ru_RU.UTF-8' -> 'ru'
            if '_' in normalized:
            ...
            if normalized in {"ru", "en"}:
                return normalized
            # fallback to 'ru' if unknown value provided
            return "ru"
        return v
